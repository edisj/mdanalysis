...
class _H5MDElement:
    ...
    def is_static(self) -> bool:
        return isinstance(self.obj, Dataset)

    def is_linear(self) -> bool:
        return isinstance(self.obj, Group) and len(self.step) == 1

    def is_time_dependent(self) -> bool:
        return isinstance(self.obj, Group) and len(self.step) > 1


def _get_elements_for(group: Group) -> List[_H5MDElement]:
    """"""
    elements = []
    ...
    def add_element(name, obj):
        if isinstance(obj, Dataset):
            if name.split("/")[-1] == "value":
                elements.append(_H5MDElement(obj.parent))
            elif name.split("/")[-1] != "step" and name.split("/")[-1] != "time":
                elements.append(_H5MDElement(obj))

    # comment about name
    group.visititems(add_element)
    return elements
...
class H5MDReader(base.ReaderBase):
    ...
    def __init__(
        ...
        comm: Optional["MPI.Comm"] = None,  # PEP 484 forward reference to type
        **kwargs,
    ):
        """
        Parameters
        ...
        **kwargs : dict
        ...
        self.h5mdfile = self.open_trajectory()
        if "h5md" not in self.h5mdfile:
            raise TypeError(
                "this is not a legal H5MD file, as it is "
                "missing the mandatory root 'h5md' group; "
                "see https://www.nongnu.org/h5md/h5md.html#h5md-root-level"
            )

        if "particles" not in self.h5mdfile:
            raise NoDataError(
                "MDAnalysis requires a particles group in the H5MD "
                "in order to load a trajectory"
            )

        if self._group is None:
            if len(self.h5mdfile["particles"]) > 1:
                raise ValueError("")
            else:
                only_group = list(self.h5mdfile["particles"])[0]
                self._particles = self.h5mdfile[f"particles/{only_group}"]
        else:
            try:
                self._particles = self.h5mdfile[f"particles/{self._group}"]
            except KeyError:
                raise ValueError(
                    f"'{self._group}' was not found in particles group"
                ) from None

        try:
            self._box = self._particles["box"]
        except KeyError:
            raise TypeError(
                "this is not a legal H5MD file, as the particle group "
                f"{self._particles.name} missing the mandatory box group; "
                "see https://www.nongnu.org/h5md/h5md.html#simulation-box"
            ) from None
        if self._box.attrs["dimension"] != 3:
            raise ValueError("MDAnalysis only supports 3-dimensional simulation boxes")

        if "edges" in self._box:
            self._edges = _H5MDElement(self._box["edges"])
            if "unit" in self._edges.attrs:
                self._verify_unit(self._edges.attrs["unit"])
        else:
            self._edges = None

        if "position" in self._particles:
            self._position = _H5MDElement(self._particles["position"])
            if "unit" in self._position.attrs:
                self._verify_unit(self._position.attrs["unit"])
                self.units["length"] = self._position.attrs["unit"]
        else:
            self._position = None

        if "velocity" in self._particles:
            self._velocity = _H5MDElement(self._particles["velocity"])
            if "unit" in self._velocity.attrs:
                self._verify_unit(self._velocity.attrs["unit"])
                self.units["velocity"] = self._velocity.attrs["unit"]
        else:
            self._velocity = None

        if "force" in self._particles:
            self._force = _H5MDElement(self._particles["force"])
            if "unit" in self._force.attrs:
                self._verify_unit(self._force.attrs["unit"])
                self.units["force"] = self._force.attrs["unit"]
        else:
            self._force = None

        if all([self._position is None, self._velocity is None, self._force is None]):
            raise NoDataError(
                "Provide at least a position, velocity"
                " or force group in the h5md file."
            )

        if "observables" in self.h5mdfile:
            self._observables = self.h5mdfile["observables"]
            self._obsv_elements = _get_elements_for("observables")
        else:
            self._observabes = None
            self._obs_elements = None

        # pulls first key out of 'particles'
        # allows for arbitrary name of group1 in 'particles'
        self._particle_group = self.h5mdfile["particles"][
            list(self.h5mdfile["particles"])[0]
        ]
    ...
    def open_trajectory(self) -> File:
        """Open the trajectory and set current frame to just before first."""

        self._frame = -1

        if isinstance(self.filename, h5py.File):
            self.h5mdfile = self.filename
            self._driver = self.h5mdfile.driver
            return self.h5mdfile

        if self._comm is not None:
            # can only pass comm argument to h5py.File if driver='mpio'
            assert self._driver == "mpio"
            self.h5mdfile = H5PYPicklable(
        ...
        self.h5mdfile = H5PYPicklable(name=self.filename, mode="r", driver=self._driver)
        return self.h5mdfile

    def _get_global_steps(self, elements) -> np.ndarray:
        global_elements = []
    ...
    def _copy_to_data(self):
        ...
        if "observables" in self.h5mdfile:
            for key in self.h5mdfile["observables"].keys():
                self.ts.data[key] = self.h5mdfile["observables"][key]["value"][
                    self._frame
                ]
    ...
    def close(self):
        """Close h5md trajectory file if it was open."""
        if self.h5md is None:
            return
        self.h5md.close()
        self.h5mdfile = None
...
class H5MDWriter(base.WriterBase):
    ...
    def __init__(
        ...
        positions: Optional[bool] = True,
        velocities: Optional[bool] = True,
        forces: Optional[bool] = True,
        timeunit: Optional[str] = None,
        lengthunit: Optional[str] = None,
        velocityunit: Optional[str] = None,
        forceunit: Optional[str] = None,
        author: Optional[str] = "N/A",
        author_email: Optional[str] = None,
        creator: Optional[str] = "MDAnalysis",
